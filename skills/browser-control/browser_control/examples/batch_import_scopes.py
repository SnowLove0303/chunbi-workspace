...
def run_batch(batch_num):
    ...
    steps = [
        ...
        {'action': 'eval_js', 'script': "(function(){var allBtns=document.querySelectorAll('button');for(var i=0;i<allBtns.length;i++){{var r=allBtns[i].getBoundingClientRect();var t=allBtns[i].textContent.trim();if(t==='Format JSON'&&r.y>340&&r.y<360){{allBtns[i].click();return 'FORMAT_OK';}}}}return 'NO_FORMAT';})();"},
        {'action': 'wait_timeout', 'ms': 1500},
        {'action': 'eval_js', 'script': "(function(){var allBtns=document.querySelectorAll('button');for(var i=0;i<allBtns.length;i++){{var r=allBtns[i].getBoundingClientRect();var t=allBtns[i].textContent.trim();if(t.indexOf('Next')>=0&&t.indexOf('Review')>=0&&r.y>690){{allBtns[i].click();return 'NEXT_OK';}}}}return 'NO_NEXT';})();"},
        {'action': 'wait_timeout', 'ms': 3000},
    ]

    r = run_actions(steps, cdp_url='http://127.0.0.1:9222')
    set_val = r['steps'][4].get('value', '')
    next_val = r['steps'][8].get('value', '')
    return True, f'Batch {batch_num}: in={len(batch_json)}, ta={set_val}, next={next_val}'
